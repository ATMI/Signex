...
sys.modules["models"] = yolov7.models
...
class Detector:
	...
	def worker(self):
		with torch.no_grad():
			...
			model = yolov7.attempt_load([self.weights], device)
			stride = int(model.stride.max())
			img_size = yolov7.check_img_size(self.img_size, s=stride)
			...
			def __detect__(image, conf_thresh=0.45, iou_thresh=0.45):
				...
				with torch.no_grad():
					pred = model(img)[0]
				pred = yolov7.non_max_suppression(pred, conf_thresh, iou_thresh, classes=None, agnostic=False)

				for i, det in enumerate(pred):  # detections per image
					if len(det):
						# Rescale boxes from img_size to im0 size
						gn = torch.tensor(image.shape)[[1, 0, 1, 0]]  # normalization gain whwh
						det[:, :4] = yolov7.scale_coords(img.shape[2:], det[:, :4], image.shape).round()

						for *xyxy, conf, cls in reversed(det):
							label = f'{names[int(cls)]} {conf:.2f}'
							yolov7.plot_one_box(xyxy, image, label=label, color=colors[int(cls)], line_thickness=1)
